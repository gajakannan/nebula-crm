...
Phase A work is documented in `planning-mds/BLUEPRINT.md` sections 3.1 through 3.5.
...
Features use 4-digit IDs (F0001, F0002, ...) and live in self-contained folders under `planning-mds/features/F{NNNN}-{slug}/`. Each feature folder contains a PRD, README, STATUS, GETTING-STARTED, and colocated story files.
...
Stories are colocated inside their feature folder: `planning-mds/features/F{NNNN}-{slug}/F{NNNN}-S{NNNN}-{slug}.md`. There is no separate top-level stories directory.
...
- ✅ `planning-mds/BLUEPRINT.md` (sections 3.1-3.5 complete)
- ✅ `planning-mds/features/REGISTRY.md` (feature index)
- ✅ Feature folders with PRDs and stories (`planning-mds/features/F{NNNN}-{slug}/`)
