Run `agents/actions/feature.md` for `{FEATURE_ID}` at `planning-mds/features/{F####-slug}` with `MODE={clean | drift-reconcile}`, `SLICE_ORDER_SOURCE={assembly-plan | override}`, and `RUN_ID={uuid4 generated at session start}`. If you use an override, keep `SLICE_ORDER` verbatim and only parallelize slices inside the same bracketed entry.

Use these tier defaults exactly:
- `clean: 1, 2`
- `drift-reconcile: 3, 4`

`PRIMARY_SPEC` is `{FEATURE_PATH}/feature-assembly-plan.md`. Start only when the plan action is already signed off, `PRIMARY_SPEC` exists, the required runtime containers are healthy per feature.md, and `python3 scripts/kg/validate.py` already exits 0.
...
5. `python3 scripts/kg/lookup.py {FEATURE_ID} --tier {start_tier} --run-id {RUN_ID} --telemetry-file .kg-state/telemetry.jsonl`
6. `{FEATURE_PATH}/**` with `PRIMARY_SPEC` treated as required reading

Treat `lookup.py` as a FIRST-PASS scope resolver only. Raw artifacts win on conflict. Open these only when lookup links them, the current gate needs them, or drift repair requires them: `planning-mds/knowledge-graph/solution-ontology.yaml`, `planning-mds/api/nebula-api.yaml`, `planning-mds/security/authorization-matrix.md`, `planning-mds/security/policies/policy.csv`, `planning-mds/knowledge-graph/*.yaml` beyond the already returned subset, and `agents/<role>/references/**` only with a `ROUTER.md` row match.

Use these commands and keep them verbatim:
- `python3 scripts/kg/workstate.py --state-file .kg-state/{FEATURE_ID}-feature.yaml init --role feature --scope {FEATURE_ID} --run-id {RUN_ID} --mode {MODE}`
...
- `hint.py <path> --run-id {RUN_ID} --telemetry-file .kg-state/telemetry.jsonl` before any Grep/Glob on code
- `blast.py <node-id> --run-id {RUN_ID} --telemetry-file .kg-state/telemetry.jsonl` before shared-semantics edits
...
- `G4.5 SIGNOFF — every Required=Yes role: verdict=PASS, reviewer, date, evidence path (not under agents/**)`
...
- `Update planning-mds/features/REGISTRY.md: status/path transitions (include archive move)`
- `Update planning-mds/features/ROADMAP.md: Now/Next/Later/Completed placement`
- `Update planning-mds/BLUEPRINT.md: feature/story status labels and links`
- `IF overall_status == "Done": move {FEATURE_PATH} to planning-mds/features/archive/{F####-slug}/ and fix impacted links`
- `Update planning-mds/knowledge-graph/feature-mappings.yaml: feature path, status, story status`
- `Update planning-mds/knowledge-graph/code-index.yaml: bindings for every new source file introduced by this feature`
...
- `python3 scripts/kg/validate.py --check-drift MUST exit 0`
...
- `python3 agents/product-manager/scripts/generate-story-index.py planning-mds/features/   (if stories changed)`
- `IF KG changed: python3 scripts/kg/validate.py --write-coverage-report`
