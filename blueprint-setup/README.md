...
1) Create `planning-mds/BLUEPRINT.md` with sections 0–2 (context, scope, constraints).
...
5) Create `planning-mds/features/` (with `REGISTRY.md` and `F{NNNN}-{slug}/` folders) for actual requirements.
...
See `blueprint-setup/examples/` for small, partial examples across different domains.

## First 4 Artifacts (Minimum)

1) `planning-mds/BLUEPRINT.md`
