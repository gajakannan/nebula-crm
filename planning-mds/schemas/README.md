...
files because they are outside F0/F1 MVP implementation scope. They are read-only reference entities
or workflow entities that will gain schemas when their features (F3–F6) are specified:

- `Account`, `MGA`, `Program` — reference lookups only; no create/update in MVP
- `Submission`, `Renewal` — detail + transitions only; schemas needed when F3/F4 land
- `WorkflowTransitionRequest`, `WorkflowTransitionRecord` — workflow payloads; schemas needed with F3/F4
